...
void Assembler::Assemble(Scanner& in_scanner, string binary_filename,
  ...
  // Read in the ascii input

  while ( in_scanner.HasNext() ) {
    string ascii = in_scanner.Next();
    ascii_strings.push_back(ascii);
  }
  ...
  for ( unsigned int i = 0; i < ascii_strings.size(); ++i ) {
    ...
    out_stream << ascii_strings.at(i) << endl;
  }

  // Read in the binary file

  std::ifstream input(binary_filename + ".bin", std::ifstream::binary);
  ...
  for (int i = 0; i < length; i+=2) {
    ...
    string converted_binary = DABnamespace::DecToBitString(value, 16);
    binary_strings.push_back(converted_binary);
  }

  // Dump the binary strings

  for ( unsigned int i = 0; i < binary_strings.size(); i++ ) {
    Utils::log_stream << binary_strings.at(i) << endl;
    out_stream << binary_strings.at(i) << endl;
  }
